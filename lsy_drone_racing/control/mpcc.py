...
global MPCC_CFG
MPCC_CFG = dict(
    # Gate-centre penalty (disable to focus on trajectory)
    Q_GATE_PEAK=0.0,
    Q_GATE_NOM=0,
    Q_GATE_DIST=0.0,
    # Contour weight: extremely high to enforce path adherence
    QC_NOM=2000,
    QC_PEAK=2000,
    SIGMA_GATE=1,  # positive to avoid divide‑by‑zero
    # Lag weight: also very high to prevent lag
    QL=1000.0,
    # Progress settings: disable speed incentive
    MU=0.4,
    RAMP_TIME=1.0,
    # Input & jerk limits: loosen slightly to avoid clipping
    REG_U=0.01,
    DVTHETA_MAX=3,
    VTHETA_SLACK=1e-2,
    # Additional penalties: disable
    R_DF=0.01,
    R_DV=1.5,
    Q_OMEGA=2,
    # MPC horizon (can remain unchanged)
    N=15,
    T_HORIZON=1.5,
)
...
def export_quadrotor_ode_model() -> AcadosModel:
    ...
    stage_cost = qc * ec_sq + ql * el_sq - mu * vtheta + df_cost + dv_cost + omega_cost
    # terminal cost should also include angular penalties
    term_cost = qc * ec_sq + ql * el_sq + omega_cost
...
def create_ocp_solver(
    ...
    ocp.constraints.ubx   = np.array([0.55,  0.55,  1.57,  1.57,  1.57, MPCC_CFG["DVTHETA_MAX"]])
...
class MPController(Controller):
    ...
    def compute_control(
        ...
        for j in range(self.N):
            # --- static reference based on current theta (no look‑ahead) ----------
            s_ref = self.last_theta
            pref = pos_on_path(s_ref)
            pref_next = pos_on_path(s_ref + 0.001)
            tangent = unit(pref_next - pref)
            self._ref_point = pref

            # -------- dynamic Gaussian contour weight qc -------------
            pos = obs["pos"]
            diff = pref - target_gate_pos  # echte Drohnen-Lage
            sigma = MPCC_CFG["SIGMA_GATE"]
            gauss = 0.0 if sigma < 1e-6 else np.exp(-0.5 * diff.dot(diff) / sigma**2)
            qc_val = MPCC_CFG["QC_NOM"] + (MPCC_CFG["QC_PEAK"] - MPCC_CFG["QC_NOM"]) * gauss
            dist = np.linalg.norm(diff)

            # Gate penalty: only apply peak at stage 0, otherwise use nominal
            dist = np.linalg.norm(diff)
            if dist < MPCC_CFG["Q_GATE_DIST"]:
                q_gate = MPCC_CFG["Q_GATE_PEAK"]
            else:
                q_gate = MPCC_CFG["Q_GATE_NOM"]
            # print(f"Distance to gate: {dist:.3f}, q_gate: {q_gate:.3f}, pos {pos}")
            ...
            p_vec = np.concatenate(
                [
                    ...
                    target_gate_pos,  # 9-11
                    [q_gate],  # 12
                    [MPCC_CFG["R_DF"], MPCC_CFG["R_DV"], MPCC_CFG["Q_OMEGA"]],
                ]
            )
            ...
            if j == 0:
                ...
                total_cost = cost_ec + cost_el + cost_mu + cost_omega + cost_gate
        ...
        p_terminal = np.concatenate(
            [
                pref_next,  # 0‑2  next reference position
                tangent,  # 3‑5  tangent
                [qc_val, ql_val, 0.0],  # 6‑8  weights (mu=0 at terminal)
                target_gate_pos,  # 9‑11 gate centre
                [MPCC_CFG["Q_GATE_NOM"]],  # 12   gate weight (nominal)
                [MPCC_CFG["R_DF"], MPCC_CFG["R_DV"], MPCC_CFG["Q_OMEGA"]],
            ]
        )
        ...
        alpha = 0.3  # tuning: larger alpha => faster update
