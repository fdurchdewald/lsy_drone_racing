...
from lsy_drone_racing.control.debug_utils import LOGDIR, get_logger
...
class MPController(Controller):
    ...
    def __init__(
        ...
        pre_post = [
            pre_post_points(gates_pos[i], gates_quat[i], self.d_gate) for i in range(len(gates_pos))
        ]
    ...
    def compute_control(
        ...
        for j in range(self.N):
            ...
            c, n, b, w, h = tunnel_bounds(self.pos_on_path, s_ref, w_nom=w_stage, h_nom=h_stage)
        ...
        # Enable warm‑start only after the first successful solve
        if self._tick == 0:
            self.acados_ocp_solver.options_set("qp_warm_start", 0)  # cold
        else:
            self.acados_ocp_solver.options_set("qp_warm_start", 2)  # full warm‑start
        ...
        for j in range(self.N):
            ...
            if hasattr(self, "_stage_centres") and j < len(self._stage_centres):
                ...
                c, n, b, _, _ = tunnel_bounds(
                    self.pos_on_path, s_ref, w_nom=self.w_nom, h_nom=self.h_nom
                )
    ...
    def update_trajectorie(self, obs: dict[str, NDArray[np.floating]] | None = None):
        for i, (pos, quat) in enumerate(zip(obs["gates_pos"], obs["gates_quat"])):
...
def pre_post_points(
    gate_pos: np.ndarray, gate_quat: np.ndarray, dist: float = 0.1
